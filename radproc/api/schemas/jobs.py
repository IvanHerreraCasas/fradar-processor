...
class TimeseriesJobRequest(BaseModel):
    ...
    start_dt: datetime = Field(..., description="Start datetime for timeseries range (UTC recommended)",
                               example="2023-10-27T10:00:00Z")
    end_dt: datetime = Field(..., description="End datetime for timeseries range (UTC recommended)",
    ...
    source: Optional[str] = Field("raw", description="Source data", examples="raw")
    version: Optional[str] = Field("", description="Version corrected data", examples="v1_0")
...
class JobStatus(BaseModel):
    ...
    status: str = Field(...,
                        description="Current job status (PENDING, RUNNING, SUCCESS, FAILURE, REVOKED)")  # Adjusted RUNNING possibility
    last_update: Optional[datetime] = Field(None,
                                            description="Timestamp of the last status update (if available)")  # Optional info
...
class JobStatusResponse(BaseModel):
    ...
    job_type: Optional[str] = Field(None,
                                    description="Type of job (e.g., timeseries, animation) - Needs storage")  # Optional: Requires storing job type with ID
...
class AccumulationJobRequest(BaseModel):
    ...
    point_name: str = Field(..., description="Unique name of the point from points.yaml", example="point_A")
    start_dt: datetime = Field(..., description="Start datetime for accumulation range (UTC recommended)",
                               example="2023-10-27T00:00:00Z")
    end_dt: datetime = Field(..., description="End datetime for accumulation range (UTC recommended)",
                             example="2023-10-28T00:00:00Z")
    interval: str = Field(...,
                          description="Pandas frequency string for accumulation interval (e.g., '1H', '15min', '1D')",
                          example="1H")
    rate_variable: Optional[str] = Field("RATE", description="Source rate variable name in timeseries CSV",
                                         example="RATE")
    source: Optional[str] = Field("raw", description="Source data", examples="raw")
    version: Optional[str] = Field("", description="Version corrected data", examples="v1_0")
    ...
    def validate_interval(cls, v: str) -> str:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid accumulation interval format: '{v}'. Use Pandas frequency string (e.g., '1H', '15min', '1D').")
        return v
...
class AnimationJobRequest(BaseModel):
    ...
    start_dt: datetime = Field(..., description="Start datetime for animation range (UTC recommended)",
                               example="2023-10-27T10:00:00Z")
    end_dt: datetime = Field(..., description="End datetime for animation range (UTC recommended)",
                             example="2023-10-27T12:00:00Z")
    extent: Optional[Tuple[float, float, float, float]] = Field(None,
                                                                description="Optional plot extent override (LonMin, LonMax, LatMin, LatMax)",
                                                                example=[-75, -73, 40, 41])
    output_format: str = Field(..., description="Desired output format extension (e.g., '.mp4', '.gif')",
                               example=".mp4")
