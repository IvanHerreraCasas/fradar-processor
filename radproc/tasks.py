...
def run_generate_point_timeseries(point_name: str, start_dt_iso: str, end_dt_iso: str, variable_override: Optional[str] = None):
    """
    ...
    """
    logger.info(f"[Task Start] Generating DB timeseries for '{point_name}', {start_dt_iso} to {end_dt_iso}, Var: {variable_override or 'Defaults'}")
    ...
    try:
        ...
        if success:
            ...
            logger.error(f"[Task Failure] Timeseries generation failed for '{point_name}' (core function returned False).")
...
# run_calculate_accumulation and run_create_animation remain largely the same,
# as they already expect/return paths and status.
